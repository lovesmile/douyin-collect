...
function buildVoiceover90({ label, profile }) {
  return [
    '0-10s：',
    `${profile.hook}如果你也是${profile.audience}，这条先别急着收藏，我们慢慢把这个问题讲清楚。`,
    '',
    '10-20s：',
    `${label}最容易被误解的地方，是大家会以为“书单越长越有用”。但真正有用的不是一下子知道很多本书，而是知道现在的自己先该读哪一本。`,
    '',
    '20-30s：',
    `如果你最近的问题是${profile.problemChoices}，那你需要的不是再收藏十个书单，而是先找到一本能对应这个问题的书。`,
    '',
    '30-40s：',
    `${profile.explain}这也是为什么我不建议一口气买很多本：书买多了不会自动解决问题，读对一本，才可能真的开始改变你的节奏。`,
    '',
    '40-50s：',
    `第一个判断标准，是看它是不是写给你现在的状态。比如你是情绪内耗，就先看情绪整理；你是亲子陪伴，就先看年龄段和使用场景；你是学习提升，就先看目录是不是够具体。`,
    '',
    '50-60s：',
    `第二个判断标准，是看目录有没有落到场景。只有大词，比如成长、自律、逆袭，但看不到具体问题的书，往往不适合马上入手。`,
    '',
    '60-70s：',
    `${profile.suitLine}反过来，如果你只是被“必读”“脱胎换骨”这种标题刺激到，其实可以先停一下。`,
    '',
    '70-80s：',
    `${profile.longScene || '真正适合普通人的读书方式，不是把书单当任务清单，而是把书当成解决一个小问题的工具。'}`,
    '',
    '80-90s：',
    `${profile.cta}先确认这本书和你的问题对得上，再决定要不要继续了解。`,
  ].join('\n');
}
...
function buildVoiceover60({ label, profile }) {
  return [
    '0-10s：',
    `${profile.hook}如果你也是${profile.audience}，这条先讲一个很具体的问题。`,
    '',
    '10-20s：',
    `${label}真正适合切入的点，不是“它有多神”，而是${profile.sellingPoint}。${profile.pain}`,
    '',
    '20-30s：',
    `${profile.bookTalk}这类书不适合用一句鸡血文案讲完，它更适合在你卡住的时候，帮你把心里的那团东西拆开一点。`,
    '',
    '30-40s：',
    `${profile.explain}你不一定要把它当成答案，更可以把它当成一个入口：先把问题说清楚，再慢慢找方法。`,
    '',
    '40-50s：',
    `${profile.suitLine}但如果你只是想马上被一句话点燃，或者希望一本书立刻改变结果，那它可能不适合你。`,
    '',
    '50-60s：',
    `${profile.cta}买书之前，先看它讲的问题是不是你正在经历的；对得上，再继续了解。`,
  ].join('\n');
}
...
function buildVoiceover30({ label, profile }) {
  return [
    ...
    `这条不讲完整内容，只讲一个点：${profile.sellingPoint}。${profile.pain}`,
    `${profile.bookTalk}`,
    `${profile.cta}`,
  ].join('\n');
}
...
function inferBookProfile(row) {
  ...
  if (/卖书|带货|读书号|书单号|自媒体|副业|达人|流程|拆解/.test(title)) {
    return {
      ...
      cta: '如果你想做这类内容，先别追求一下子讲很多书，先把一本书和一个问题讲清楚。',
      bookTalk: '它提醒的是：书籍内容真正打动人的地方，不是“推荐了多少本”，而是能不能把一个人的真实困惑讲明白。',
      problemChoices: '不知道怎么选书、不知道怎么讲书、或者担心内容和商品对不上',
  ...
  if (/允许一切发生/.test(title)) {
    return {
      ...
      bookTalk: '它更像是在提醒你：允许事情发生，不等于放弃选择，而是先承认事情已经来了，再决定自己下一步怎么走。',
      problemChoices: '反复后悔、反复比较、反复复盘一段关系',
  ...
  if (/妈妈|当妈|孩子|育儿|童书|亲子|教辅|成绩|学习/.test(title)) {
    return {
      ...
      cta: '如果你也在这个阶段，可以先从一本最贴近当下状态的书开始，不用一次逼自己读完一整套书单。',
      bookTalk: '它真正想讲的不是“妈妈必须读多少书”，而是当生活被家庭填满以后，人还是需要有一点属于自己的精神空间。',
      problemChoices: '亲子陪伴、妈妈自我恢复、孩子阅读习惯',
  ...
  if (/三本|3本|十本|10本|书单|好书|不得不看|脱胎换骨/.test(title)) {
    return {
      ...
      cta: '如果你也经常收藏书单，可以先选一本最对应当前问题的书，看目录对不对，再决定要不要继续。',
      bookTalk: '一份真正有用的书单，不是把所有好书堆在一起，而是帮你从“我该看什么”变成“我现在先看哪一本”。',
      problemChoices: '情绪、表达、学习、行动力',
  ...
  if (/内耗|拧巴|焦虑|情绪|接纳|人生|选择/.test(title)) {
    return {
      ...
      cta: '如果你也在这种状态里，可以先找一本目录真正对应这个问题的书，慢慢看，不用急着被治好。',
      bookTalk: '它适合讲成一种陪伴感：不是告诉你马上变好，而是先让你知道，原来这种反复消耗是可以被看见的。',
      problemChoices: '内耗、后悔、情绪拉扯',
  ...
  return {
    ...
    bookTalk: '一本书能不能打动人，关键不是标题多漂亮，而是它有没有刚好对应你现在遇到的问题。',
    problemChoices: '情绪、学习、亲子、表达、行动力',
